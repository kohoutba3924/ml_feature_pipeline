...
from ml_feature_pipeline.ingestion.lcdv2 import hourly_prior_years
...
def mock_target_wbans(monkeypatch):
    ...
    """
    monkeypatch.setattr(hourly_prior_years, "TARGET_STATION_WBANS", {"99999", "14839"})
...
def mock_settings(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(hourly_prior_years, "settings", DummySettings())
...
def mock_ingestion_state(monkeypatch):
    ...
    dummy = DummyState()
    monkeypatch.setattr(hourly_prior_years, "ingestion_state", dummy)
    return dummy
...
def mock_session(monkeypatch):
    ...
    """
    mock = MagicMock()
    monkeypatch.setattr(hourly_prior_years, "SESSION", mock)
    return mock
...
def test_extract_wban_from_filename():
    assert hourly_prior_years.extract_wban_from_filename("01001099999.csv") == "99999"
    assert hourly_prior_years.extract_wban_from_filename("72640014839.csv") == "14839"
...
def test_get_years_to_ingest(monkeypatch):
    ...
    monkeypatch.setattr(hourly_prior_years, "date", DummyDate)
    monkeypatch.setattr(hourly_prior_years.settings, "LCDV2_HISTORICAL_YEARS", 2)

    assert hourly_prior_years.get_years_to_ingest() == [2024, 2025]


def test_list_hourly_files_regex_parsing(mock_session, mock_settings):
    ...
    files = hourly_prior_years.list_hourly_files_for_year(2024)
...
def test_missing_file_detection(
    ...
    output_dir = tmp_path / "raw" / "lcdv2" / "hourly" / "2024"
    ...
    # Run ingestion
    results = hourly_prior_years.ingest_hourly_raw()
...
def test_download_worker(mock_session, tmp_path, mock_settings):
    ...
    result = hourly_prior_years._download_worker(2024, "01001099999.csv", output_dir)
...
def test_full_ingestion_flow(
    ...
    out_dir = tmp_path / "raw" / "lcdv2" / "hourly" / "2024"
